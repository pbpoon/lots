...
class ProductCreateOrder(models.Model):
    _name = 'product.create.order'
    ...
    cost_by = fields.Selection(selection=COST_BY_SELECTION, string='成本计算方式', default='t')
    order_line_ids = fields.One2many('product.create.order.line', 'order_id', string='订单明细')
...
class ProductCreateOrderLine(models.Model):
    _name = 'product.create.order.line'
    ...
    product_id = fields.Many2one('product.product', '产品')
    order_id = fields.Many2one('product.create.order', 'order', ondelete='cascade', required=True)
    ...
    def create(self, vals):
        ...
        if not vals.get('product_id'):
            if not order.product_id:
                raise exceptions.ValidationError('请选择产品名称!')
            vals.update({'product_id': order.product_id.id})
        ...
        product_block_id = self.env['product.block'].create(vals)
        lot = self.env['stock.production.lot'].create({'name': vals.get('name', product_block_id.name),
                                                       'product_id': vals.get('product_id'),
                                                       'product_block_id': product_block_id.id})
        vals['lot_id'] = lot.id
        return super(ProductCreateOrderLine, self).create(vals)
