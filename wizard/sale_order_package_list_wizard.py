...
class PackageList(models.TransientModel):
    ...
    lot_id = fields.Many2one('stock.production.lot', '荒料编号', required=True, readonly=True)
    product_id = fields.Many2one('product.product', '产品', required=True, readonly=True)
    ...
    slab_ids = fields.Many2many('product.slab', string='板材规格')
    ...
    def Apply(self):
        ...
        package_list = self.env['package.list'].create({
            'lot_id': self.lot_id.id, 'product_id': self.product_id.id, 'slab_ids': [(6, 0, slab_lst)]
        })
    ...
    def button_add(self):
        self.ensure_one()
        new_slab_ids = self.env['product.slab']
...
class PackageLine(models.TransientModel):
    ...
    package_wizard_id = fields.Many2one('package.list.wizard', '码单')
